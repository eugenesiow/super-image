...
def output_singles(input_dir, eval_file, scale, model_type='edsr'):
    device = 'cuda:0' if torch.cuda.is_available() else 'cpu'
    eval_dataset = EvalDatasetH5(eval_file)
    eval_dataloader = DataLoader(dataset=eval_dataset, batch_size=1)
    if model_type == 'edsr':
    ...
    for i, data in enumerate(eval_dataloader):
        ...
        cv2.imwrite('../../../super-image-models/results/' + model_type + '_' + str(i) + '_' + str(scale)
                    + '_single_bicubic.png', img_resize)
        cv2.imwrite('../../../super-image-models/results/' + model_type + '_' + str(i) + '_' + str(scale)
                    + '_single_pred.png', pred)
...
output_image('../../../super-image-models/drln', '../../../super-image-models/test/Set5_x4.h5', 4, model_type='drln')
